<template>
    <Toolbar v-on:close="$router.back()" v-bind:title="$t('settings.push_notifications')"></Toolbar>
    <SettingsDesc v-bind:text="$t('settings.forms.notif_settings.push_notifications_helper')"></SettingsDesc>
    <div class="mb-3">
...
<script>
    ...
    export default defineComponent({
        setup: function() {
            ...
            onMounted(async () => {
                await colibriAPI().userSettings().getFrom('notifications/push/settings').then((response) => {
            ...
            const submitForm = async () => {
                if (state.isSubmitting === false) {
                    ...
                    await colibriAPI().userSettings().with(formData.value).putTo('notification/push/update').then((response) => {
